...
export default function OwnerDetailsScreen() {
  ...
  return (
    <Page>
      ...
      <Text style={styles.label}>Owner Contact</Text><TextInput style={styles.input} value={form.owner_contact || ''} onChangeText={(v) => set('owner_contact', v)} />
      ...
      {!!(form.tenant_name || '').trim()
        ...
        : <TouchableOpacity style={styles.linkBtn} onPress={() => navigation.navigate('AddTenant', { propertyId, block: form.block, owner_name: form.owner_name || '' })}><Text style={styles.linkTxt}>Add Tenant</Text></TouchableOpacity>
      }
